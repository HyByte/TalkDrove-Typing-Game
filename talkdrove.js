...
ig.module('game.menus.interstitial').requires('game.menus.base').defines(function() {
    ...
    MenuInterstitial = Menu.extend({
        ...
        scale: 0.75,
        banner: new ig.Image('media/Hamza-works.png'),
        init: function() {
            this.parent();
...
ig.module('game.menus.detailed-stats').requires('game.menus.base', 'game.xhr').defines(function() {
    DetailedStats = ig.Class.extend({
        ...
        saveScoresScreenshot: function(callback) {
            ...
            ctx.fillStyle = '#ccc';
            ctx.font = '28px sans-serif';
            ctx.fillText(' –  http://HyHamza.vercel.app/', 515, 38);
        ...
        shareFacebook: function() {
            ...
            this.saveScoresScreenshot(function(response) {
                var imageUrl = 'http://HyHamza.vercel.app/' + response.file;
                shareWindow.location.href = 'https://www.facebook.com/dialog/feed' + '?app_id=112268002182064' + '&display=popup' + '&caption=' + encodeURIComponent('My ZTYPE Scores') + '&link=' + encodeURIComponent('http://HyHamza.vercel.app/') + '&picture=' + encodeURIComponent(imageUrl);
            });
        },
...
ig.module('game.menus.title').requires('game.menus.base', 'game.menus.detailed-stats', 'game.ease').defines(function() {
    ...
    MenuItemOnlineVersion = MenuItem.extend({
        getText: function() {
            return 'Developer site?';
        },
        ok: function() {
            window.location.href='http://HyHamza.vercel.app/';
        },
    });
    MenuItemAboutTrainer = MenuItem.extend({
        getText: function() {
            return "Hamza's GitHub";
        },
        ok: function() {
            window.location.href='https://github.com/HyHamza';
        },
    });
    MenuTitle = Menu.extend({
        ...
        scroll: 0,
        background: new ig.Image('media/background/stars.jpg'),
        ztype: new ig.Image('media/title/talkdrove-logo.png'),
        phoboslab: new ig.Image('media/title/logo2.png'),
        ...
        update: function() {
            ...
            if (ig.input.mouse.y < 104 && ig.input.pressed('click')) {
                if (window.ejecta) {
                    ejecta.openURL('https://HyHamza.vercel.app');
                } 
                else {
                    window.open('https://HyHamza.vercel.app');
                }
                return;
            }
            this.parent();
